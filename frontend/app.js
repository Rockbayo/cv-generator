...
const skillsFormTitle = document.getElementById('skillsFormTitle');
...
skillsNavBtn.addEventListener('click', () => showSection('skills'));

// Event listeners de habilidades
addSkillBtn.addEventListener('click', showSkillsForm);
skillForm.addEventListener('submit', handleSkillsSubmit);
cancelSkillBtn.addEventListener('click', hideSkillsForm);
...
function showSection(section) {
    ...
    if (section === 'experiences') {
        ...
        skillsNavBtn.classList.add('active');
...
async function loadSkills() {
    ...
    try {
        const response = await fetch(${API_BASE}/usuario//habilidades);
        const result = await response.json();
        
        if (result.status === 'success') {
            currentSkills = result.habilidades;
            renderSkills();
        }
    } catch (error) {
        console.log('Error cargando habilidades:', error);
    }
}

function renderSkills() {
    if (currentSkills.length === 0) {
        skillsList.innerHTML = '<p class="no-data">No hay habilidades registradas</p>';
        return;
    }
    
    // Agrupar por tipo
    const tecnicas = currentSkills.filter(skill => skill.tipo === 'Técnica');
    const blandas = currentSkills.filter(skill => skill.tipo === 'Blanda');
    
    let skillsHTML = '';
    
    if (tecnicas.length > 0) {
        skillsHTML += '<div class="skills-category"><h4>Habilidades Técnicas</h4>';
        skillsHTML += tecnicas.map(skill => createSkillHTML(skill)).join('');
        skillsHTML += '</div>';
    }
    
    if (blandas.length > 0) {
        skillsHTML += '<div class="skills-category"><h4>Habilidades Blandas</h4>';
        skillsHTML += blandas.map(skill => createSkillHTML(skill)).join('');
        skillsHTML += '</div>';
    }
...
function createSkillHTML(skill) {
    const levelClass = skill.nivel.toLowerCase().replace('á', 'a').replace('é', 'e');
    const levelWidth = {
        'basico': '25%',
        'intermedio': '50%', 
        'avanzado': '75%',
        ...
        <div class="skill-item">
            <div class="skill-header">
                ...
                <div class="skill-actions">
                    <button class="btn-small btn-edit" onclick="editSkill()">Editar</button>
                    <button class="btn-small btn-delete" onclick="deleteSkill()">Eliminar</button>
                </div>
            </div>
...
function showSkillsForm() {
    editingSkillId = null;
    skillsFormTitle.textContent = 'Nueva Habilidad';
    skillForm.reset();
    skillsForm.style.display = 'block';
    saveSkillBtn.textContent = 'Guardar Habilidad';
}
...
async function handleSkillsSubmit(e) {
    ...
    const formData = {
        nombre: document.getElementById('nombreHabilidad').value,
        nivel: document.getElementById('nivelHabilidad').value,
        tipo: document.querySelector('input[name="tipoHabilidad"]:checked').value
    };
    ...
    try {
        ...
        if (editingSkillId) {
            response = await fetch(${API_BASE}/habilidad/, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json'
                },
            ...
            response = await fetch(${API_BASE}/usuario//habilidades, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
        ...
        if (result.status === 'success') {
            ...
            updatePreview();
            hideSkillsForm();
            showMessage(editingSkillId ? 'Habilidad actualizada exitosamente' : 'Habilidad agregada exitosamente', 'success');
        } else {
            showMessage('Error: ' + result.message, 'error');
        }
    } catch (error) {
...
async function loadSkills() {
    ...
    try {
        const response = await fetch(`${API_BASE}/usuario/${currentUser.id}/habilidades`);
        const result = await response.json();
...
function renderSkills() {
    ...
    // Ordenar: Técnicas primero, luego por nivel (Experto -> Básico)
    const levelOrder = { 'Experto': 4, 'Avanzado': 3, 'Intermedio': 2, 'Básico': 1 };
    const sortedSkills = [...currentSkills].sort((a, b) => {
        if (a.tipo !== b.tipo) {
            return a.tipo === 'Técnica' ? -1 : 1;
        }
        return levelOrder[b.nivel] - levelOrder[a.nivel];
    });
    
    const skillsHTML = sortedSkills.map(skill => {
        const levelClass = skill.nivel.toLowerCase().replace('á', 'a');
        const typeClass = skill.tipo.toLowerCase();
                ...
                <div class="skill-header">
                    <div class="skill-info">
                        ...
                        <div class="skill-meta">
                            <span class="skill-type">${skill.tipo}</span>
                            <span class="skill-level ${levelClass}">${skill.nivel}</span>
                        </div>
                    </div>
                    <div class="skill-actions">
                        <button class="btn-small btn-edit" onclick="editSkill(${skill.id})">Editar</button>
                        <button class="btn-small btn-delete" onclick="deleteSkill(${skill.id})">Eliminar</button>
                    </div>
                </div>
                <div class="skill-progress">
                    <div class="progress-bar">
                        <div class="progress-fill ${levelClass}" style="width: ${getLevelPercentage(skill.nivel)}%"></div>
                    </div>
                </div>
            </div>
        `;
...
function getLevelPercentage(nivel) {
    const percentages = { 'Básico': 25, 'Intermedio': 50, 'Avanzado': 75, 'Experto': 100 };
    return percentages[nivel] || 0;
}

function showSkillForm() {
    editingSkillId = null;
    skillFormTitle.textContent = 'Nueva Habilidad';
    skillFormElement.reset();
...
async function handleSkillSubmit(e) {
    ...
    try {
        ...
        if (editingSkillId) {
            response = await fetch(`${API_BASE}/habilidad/${editingSkillId}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(formData)
            });
        } else {
            response = await fetch(`${API_BASE}/usuario/${currentUser.id}/habilidades`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(formData)
            });
...
window.editSkill = async function(skillId) {
    ...
    skillFormTitle.textContent = 'Editar Habilidad';
    
    document.getElementById('nombreHabilidad').value = skill.nombre;
    document.getElementById('nivelHabilidad').value = skill.nivel;
    document.querySelector(`input[name="tipoHabilidad"][value="${skill.tipo}"]`).checked = true;
    
    skillForm.style.display = 'block';
    saveSkillBtn.textContent = 'Actualizar Habilidad';
}

window.deleteSkill = async function(skillId) {
    if (!confirm('¿Estas seguro de eliminar esta habilidad?')) return;
    
    try {
        const response = await fetch(`${API_BASE}/habilidad/${skillId}`, {
            method: 'DELETE'
        });
        ...
        if (result.status === 'success') {
            await loadSkills();
            updatePreview();
            showMessage('Habilidad eliminada exitosamente', 'success');
        } else {
            showMessage('Error: ' + result.message, 'error');
        }
    } catch (error) {
